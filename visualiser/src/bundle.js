...
Person.prototype = {
	setMother: function setMother(mother) {
		this.mother = mother;
	},

	setFather: function setFather(father) {
		this.father = father;
	},
	...
	addReign: function addReign(reign) {
		this.reigns.push(reign);
	},

	getRelatives: function getRelatives() {
		var relatives = [];
...
LifelinePlotComponent.prototype = {
	...
	generateLifelines: function generateLifelines() {
	...
	generateBirthlines: function generateBirthlines() {
	...
	adjustOverallPosition: function adjustOverallPosition(adjustment) {
		for (var link in this.link2lifeline) {
			var currLifeline = this.link2lifeline[link];
	...
	evaluatePositions: function evaluatePositions(evaluationStack) {
	...
	assignStartingPositions: function assignStartingPositions(focalLifeline) {
	...
	getLifelinesAtPos: function getLifelinesAtPos(currGridPos) {
		var lifelinesAtPos = [];
		for (var link in this.link2lifeline) {
			var currLifeline = this.link2lifeline[link];
	...
	adjustPositionsForOverlap: function adjustPositionsForOverlap() {
	...
	getMinGridPos: function getMinGridPos() {
		var minPos = Infinity;
		for (var link in this.link2lifeline) {
			var currLifeline = this.link2lifeline[link];
	...
	getMaxGridPos: function getMaxGridPos() {
		var maxPos = -Infinity;
		for (var link in this.link2lifeline) {
			var currLifeline = this.link2lifeline[link];
	...
	// argument.
	assignRelativePositions: function assignRelativePositions() {
	...
	selectFocalPerson: function selectFocalPerson() {
...
LifelinePlot.prototype = {
	calculateBoundaryYears: function calculateBoundaryYears() {
	...
	getYearCoord: function getYearCoord(year) {
	...
	// into consideration:
	fraction2position: function fraction2position(fractionOfWidth) {
	...
	convertXpos: function convertXpos(gridPos) {
	...
	convertYpos: function convertYpos(yPos) {
		// XXX FIXME: Adjust to include margin:
		return this.getYearCoord(yPos);
	},

	displayLifelines: function displayLifelines() {
		// Overview:
		/*
...
Lifeline.prototype = {
	...
	drawReigns: function drawReigns() {
		...
		for (var reignIdx = 0; reignIdx < this.person.reigns.length; reignIdx++) {
			...
			if (lineagesToShowLinks.has(currLineageLink)) {
				...
				var xStart = this.getXstart();
				var boxWidth = this.hostComponent.hostPlot.boxWidth;
				reignRectangle = this.hostComponent.hostPlot.svgTarget.append("rect").attr("x", xStart).attr("y", yStart).attr("width", boxWidth).attr("height", yEnd - yStart).attr("fill", "yellow").attr("stroke", "yellow");
				this.reignBars.push(reignRectangle);
			}
		}
	},

	select: function select(depth) {
